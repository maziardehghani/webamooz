...
<div class="col-12 bg-white">
    ...
    <form action="{{route('dashboard.users.profile.update',auth()->user()->id)}}" method="post" class="padding-30" enctype="multipart/form-data">
        ...
        @enderror
        <input name="username"  type="text"  class="text" value="{{auth()->user()->username}}">
        @error('username')
        <div class=" text-danger colorRed">
            <strong>{{ $message }}</strong>
        </div>
        ...
        <input name="cardNumber"  type="text"  class="text" placeholder="شماره کارت بانکی">
        @error('cardNumber')
        <div class=" text-danger colorRed">
            <strong>{{ $message }}</strong>
        </div>
        @enderror
        <input name="shaba"  type="text"  class="text" placeholder="شماره شبا بانکی">
        @error('shaba')
        <div class=" text-danger colorRed">
            <strong>{{ $message }}</strong>
        </div>
